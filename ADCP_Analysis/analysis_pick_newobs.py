...
### ssumed the direction in adcp files are cartesian ###
...
dfeu=dfeM*np.cos(dfeD/180*np.pi)
dfev=dfeM*np.sin(dfeD/180*np.pi)

dfmu=dfmM*np.cos(dfmD/180*np.pi)
dfmv=dfmM*np.sin(dfmD/180*np.pi)

dfwu=dfwM*np.cos(dfwD/180*np.pi)
dfwv=dfwM*np.sin(dfwD/180*np.pi)
...
ax[1].invert_yaxis()
ax[1].grid(linewidth=0.3)
plt.savefig('HA2103_General_Profiles_.png',dpi=300)
...
ax[1].set(xlim=[-0.45,0.45])
ax[1].set(ylim=[0.00,8.0])
ax[1].set_xlabel('Speed (m/s)', size=10)
ax[1].set_ylabel('Depth (m)', size=10)
ax[1].invert_yaxis()
ax[1].grid(linewidth=0.3)
plt.savefig('HA1104_General_Profiles_.png',dpi=300)
...
ax.set_legend()
plt.suptitle('HA1104 Field Bottom Layer Current Rose')
...
ax.set_legend()
plt.suptitle('HA1104 Upper Layer Current Rose')
