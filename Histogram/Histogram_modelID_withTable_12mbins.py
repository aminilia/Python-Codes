...
##########################3/14/2024#################################
####################################################################
########program to plot multiple models stats on a histogram########
####################################################################
import numpy as np
import pandas as pd
...
for i in range(len(model_list)):
    ax1.bar(bins[:-1]+bar_width*i-len(model_list)/2*bar_width, abs(mean_bs[i+1]), width=bar_width, label=model_list[i])
...
plt.savefig('hist_sal_with_table_12m.png',dpi=300)
...
ax1 = fig.add_subplot(gs[0])
for i in range(len(model_list)):
    ax1.bar(bins[:-1]+bar_width*i-len(model_list)/2*bar_width, abs(mean_bt[i+1]), width=bar_width, label=model_list[i])
...
plt.tight_layout()
plt.subplots_adjust(hspace=0.10)  # Reduce space between histogram and table
plt.savefig('hist_temp_with_table_12m.png',dpi=300)
